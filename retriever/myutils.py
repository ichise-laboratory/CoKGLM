...
relation_path = "../kg/kg_relation.txt"
entity_path = "../kg/kg_entity.txt"
kge_path = "../kg"
ext_entities_file = "../kg/ext_entities.txt"


def ext_entities_dic(list_file):
    ext_entities = {}
...
class Annotator:
    def __init__(self, freebase_file: str, kge_dir: str, ext_entities_file: str):
        ...
        self.ext_entities = ext_entities_dic(ext_entities_file)
...
class KnowledgeGraphEmbedding:
    def __init__(self, kge_path: str, *dataset_paths, init_mean: float = 0.0, init_std: float = 1.0):
        ...
        self.node2id, self.id2node = self._load_ids("kg_entity.txt")
        self.rel2id, self.id2rel = self._load_ids("kg_relation.txt")

        self.node_embds = self._load_and_stack("../kg/TransE_768_result/ent_embeddings")
        self.rel_embds = self._load_and_stack("../kg/TransE_768_result/rel_embeddings")
        """
        if dataset_paths:
            new_ents = set() 
            new_rels = set()
    ...
    def _load_and_stack(self, pickle_file: str):
        # breakpoint()
        ...
        with open(dataset_path, "r", encoding="utf-8") as f:
            for line in f: 
                ...
                if dialogue["knowledge_base"]:
                    for subj, pred, obj in dialogue["knowledge_base"]:
