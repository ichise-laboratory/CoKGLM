...
def build_dataset(tripleid, in_path, out_path, mod, Graph, name2entity, kg_triple_id, entity2name, relation2name, filter_no_english=False):
    ...
    tokenizer.add_tokens(['<sep>','<triple>','<user>','<assistant>','<response>', '<ent>', '</ent>'])
    ...
    with open(in_path, 'r') as f_in, open(out_path,"w") as fout:
        ...
        writer.writerow(['entity_relation_ids','named_entity', 'named_entity_id', 'memory_bank', 'entity_triple_counts', 'history_response', 'corrupt_entity', 'original_entity', 'hallucination_tags', "data_id"])
        ...
        for idx, l_in in tqdm(enumerate(reader)):
            ...
            MAX_TRIPLES_PER_ENTITY = 2  # the number of triples to retrieve for each entity (doesn't need for LLM only setting)
            
            original_corrupt_response = corrupt_response.lower()
            
            for entity in matched_entities:
                tagged_lower_entity = None
                tagged_corrupt_response = None
                ...
                tagged_lower_entity = "<ent>" + lower_entity + "</ent>"  # Add tags before and after the detected entities
                tagged_corrupt_response = original_corrupt_response.replace(lower_entity, tagged_lower_entity)
                ...
                for node in Graph.nodes():
                    ...
                    if fuzz.ratio(lower_entity, node.lower()) >= 99:
                        ...
                        print("lower_entity_ids:", lower_entity_ids)
                ...
                if best_id is not None:
                    ...
                    best_entity_id[lower_entity] = int(best_id) + 1  # <pad> (assigned an ID of 0) will later be added to entity2id
                    lower_entities.append(lower_entity)
                else:          
                    print(f"No valid best_id found for entity {lower_entity}")
                    continue
                                        
                entity_relation_ids = [0]*text_prefix_len 
                memory_bank = []
                triples = []
                
                if_only_english = True
                all_entities = set()
                #entity_embedding = nlp(lower_entity).vector
                ...
                for triple, triple_ids in triple_list:
                    ...
                    """
                if not memory_bank:
                    print(f"No valid triples found for entity {lower_entity} {best_id}")
                    continue
                    
                input_text = text_prefix
                
                if not if_only_english:
                    print('no only', idx, input_text)
                    continue                  
                
                corrupt_response, all_results = match_entities(lower_entities, tagged_corrupt_response, embedder, tagger, if_replace=True)
                corrupt_response_entity_relation_ids = get_tokenized_idx(corrupt_response, all_results, best_entity_id, tokenizer, mod)
                ...
                entity_relation_ids += text_entity_relation_ids
...
# Check whether the best_id (linked entity id) matches the IDs in entity_relation_id
def check_entity_id(path):
    with open(path) as f:
        ...
        for i, row in enumerate(reader):
            ...
            entity_ids2 = set([0]) 
            best_id = ast.literal_eval(row[2])
            if isinstance(best_id, int):  
                best_id = [best_id]
            elif not isinstance(best_id, list):  
                raise TypeError(f"Unexpected data type in row[{i}][2]: Expected list, got {type(best_id).__name__}")
            for id in best_id:
                entity_ids2.add(id)
