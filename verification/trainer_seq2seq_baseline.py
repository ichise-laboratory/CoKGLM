...
class Seq2SeqTrainer(Trainer):
    ...
    def prediction_step(
        ...
        with torch.no_grad(): # モデルを入力データで実行し、出力を取得する。勾配計算を避けてメモリ使用量を減らし、推論速度を向上させる
            if self.use_amp:
                with autocast():
        ...
        if self.args.prediction_loss_only: # メソッドは損失のみを返し、予測されたトークンやラベルは返さない→実行時間を減らす
    ...
    def _pad_tensors_to_max_len(self, tensor, max_length): # トークンやラベルがmax_lengthに満たない場合、指定された最大長までパディング
