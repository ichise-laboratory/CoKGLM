...
class BartForConditionalGeneration(BartPretrainedModel):
    ...
    def __init__(self, config: BartConfig, entity_relation_weight):
        ...
        self.classification_head = nn.Sequential(nn.Linear(config.d_model, 2)) ## # Fully connected layer for binary classification (2 classes)
        self.init_weights()
    ...
    def forward(
        ...
        masked_lm_loss = None
        if labels is not None:
            # outputs[0]: torch.Size([4, 800, 768])
            pooled_output = outputs[0].mean(1) 
            classification_logits = self.classification_head(pooled_output)
            labels = labels.long()
            
            loss_fct = CrossEntropyLoss()
            masked_lm_loss = loss_fct(classification_logits, labels)
        

        if not return_dict:
            output = (classification_logits,) + outputs[1:]
            return (masked_lm_loss,) + output if masked_lm_loss is not None else output
        
        
        """
        fc_layer1 = nn.Linear(3,3).to("cuda")
        bn = nn.BatchNorm1d(3).to("cuda")
        fc_layer2 = nn.Linear(3, 1).to("cuda")
       
        x = fc_layer1(outputs.encoder_last_hidden_state)
        x = bn(x)
        output = torch.sigmoid(fc_layer2(x))
        return output
        """
